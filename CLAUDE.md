...
## ⚠ SCOPE — read this before anything below it

**Everything below describes the LEGACY hand-authored tree (`docs/grammar/`,
`docs/expressions/`, `docs/dialogues/`, `docs/patterns/`, `docs/questions/`)
and is still correct THERE.** Those pages really do carry YAML frontmatter,
really do use `<ruby>` furigana, and really are listed by hand in an
`index.md`.

**It is WRONG for `docs/deck/`, which is where all current work happens.**
Deck pages are GENERATED by `jp-deck/scripts/build_site.py` from prose files in
`content/deck/grammar/<slug>.md`. For anything under `docs/deck/`:

- **`docs/deck/grammar/index.md` is generated and carries a DO-NOT-HAND-EDIT
  banner.** Do not add your page to it. Do not edit it at all.
- **Furigana is `語[よみ]` shorthand, not `<ruby>`.** Zero deck pages use
  `<ruby>`; the generator expands the shorthand.
- **The nav is generator-owned.** Do not register a deck page in `mkdocs.yml`.
- **You author `content/deck/grammar/<slug>.md`, never the `docs/` output.**

**The authorities for deck pages are `jp-deck/reference-site/PAGE-SPEC.md`
(content) and `PRESENTATION-SPEC.md` (visual), not this file.** Where this file
and those specs disagree about a deck page, the specs win and this file is out
of date.

Found 2026-08-27 by the `#245` re-test's `kara` worker, which read this file,
noticed it described a superseded system, followed the specs instead and
...
Personal MkDocs-based Japanese grammar reference site. Tokyo casual register, targeting N3–N4 level. Hosted on Cloudflare Workers Static Assets.

**Deploy: push to `main` deploys via Cloudflare Workers Static Assets
(`wrangler.jsonc`), NOT GitHub Pages — that workflow was deleted at `6f919c0`.
Push IS the deploy (`#331`).**
...
- **Deploy**: push to `main` — Cloudflare Workers Static Assets
  (`wrangler.jsonc`). **Not** GitHub Pages; see the deploy note above (`#331`).

## Directory structure

```
docs/
  deck/                       ← GENERATED by jp-deck/scripts/build_site.py — never hand-edit
  grammar/
...
content/deck/grammar/<slug>.md  ← what you actually author for deck pages
```
