Somebody's wallet slips out of their bag on the platform in front of you, and you say 財布、落ちましたよ — you've dropped your wallet.

Without よ the sentence still reports that the wallet fell. With it, the sentence is addressed: this is for you, and you did not have it.

That is よ's whole job. It marks the content as the speaker's to supply and the listener's to receive.

Everything else よ gets described as doing — emphasising, insisting, sounding pushy — follows from what happens when that assumption is wrong. Handing somebody something they already had reads as a correction.

Before adding よ, ask whether the listener already has this. If they do not, よ is the ordinary way to pass it over. If they do, expect it to land as a reminder rather than as news.

One thing a page cannot show: よ's contour carries a great deal. A falling よ sets the fact down; a rising よ appeals. The sections below say which one they mean.

@@ tldr

よ marks a statement as something the speaker is supplying and the listener does not yet have.
Everything else it is said to do — emphasis, insistence, pressure — comes of using it where the listener already knew.
It carries no politeness of its own and follows です and ます unchanged.

@@ main

**Form:** `plain sentence + よ` · `N / な-adj + だよ` · `です・ます + よ`

- **よ addresses the statement to somebody** — そこ、まだ濡れてるよ — that's still wet — says it to the person who is about to sit down.
- **It says the content is new to the listener,** not that it is important, urgent or loud. An ordinary fact takes よ as readily as a warning does.
- **It carries no politeness of its own.** 危ないですよ — that's dangerous — is polite because of です; よ neither raises nor lowers the register it lands in.
- **Negation does not change its job** — 今日は行かないよ — I'm not going today — hands over a negative fact exactly as よ hands over a positive one.
- **Sentence-final よ takes the whole sentence in its scope.** It says nothing about which part of the sentence is new; it addresses the whole statement to the listener.

```ex
その席[せき]、予約[よやく]入[はい]ってますよ。
en: That seat's booked.
lit: as for that seat, a reservation has gone in
note: Nothing about the seat shows it, so the listener has no way of knowing.
  よ is what makes this a thing said to this person at the moment it is useful
  to them, rather than a description of the seat.
bd: その席、|that seat, / 予約|a booking / 入ってます|has gone in / よ*|[I'm telling you]
pat: statement + よ, information the listener could not have had
reg: casual|その席、予約入ってるよ。 / polite|その席、予約入ってますよ。
```

### What よ adds to a plain statement

```eg
addressed: 明日[あした]、休[やす]みだよ | Tomorrow's a day off, just so you know.
unaddressed: 明日[あした]、休[やす]みだ | Tomorrow is a day off.
```

The fact is identical in both. The second states it; the first gives it to somebody, and it only makes sense if there is somebody there who did not already have it.

That is why よ is everywhere in conversation and almost absent from writing that is not addressed to a reader. A timetable does not tell you anything. A person does.

```dialog
note: what licenses よ — A is asking about something only B can see, so B's answer is B's to supply.
A: レジ、並[なら]んでる？ | Is there a queue at the till?
B: 空[す]いてるよ。今[いま]なら大丈夫[だいじょうぶ]。 | There's no queue. Now would be fine.
```

### Rising and falling

A falling よ is the informing one. It sets the fact down in front of the listener and stops.

A rising よ appeals. It reaches for the listener's attention, or for their agreement to act, and it is this contour that carries urging, coaxing and complaint.

The written sentence is the same either way, which is why よ gets described both as neutral and as pushy. Both descriptions are of contours rather than of the particle.

そろそろ行くよ — we're leaving soon — said flat is a notification. The same words on a rising contour, after the listener has stalled twice, are a complaint.

### Urging with よ

- **On a volitional it turns a proposal into encouragement** — そろそろ決めようよ — come on, let's decide — presses for the action rather than asking about it.
- **On a request it adds pressure** — 早く来てよ — hurry up and get here — is as much a complaint as a request.
- **The pressure is not in the particle by itself.** よ presents the action as the direction the speaker wants things to take; whether that reads as warm or as pushy is settled by the relationship and by what has already been said.

-> Compare: [[ikoukei#ikoukei-yo]] — 意向形＋よ, the volitional carrying this same よ

```dialog
note: what licenses よ — A's assumption is wrong, so the true state of things is B's to supply. Putting a false assumption right is よ's ordinary informing job, not its edgy one.
A: もう終[お]わったよね？ | It's finished already, right?
B: まだ途中[とちゅう]だよ。 | It's only half done.
```

### Choosing how to close the sentence

Ending a Japanese sentence is a choice, and よ is one of five ordinary answers. The rows run from the plainest close to the most conversational.

| how you close | what it commits you to | how it lands |
|---|---|---|
| nothing — a plain statement | states the fact, addressed to nobody in particular | neutral; the default in writing and in narration |
| 〜よ — I'm telling you | presents the fact as yours to supply | very common in speech; pushes when the listener already knew |
| [[ne#agreement]] — isn't it | presents the fact as the listener's to confirm | very common in speech; assumes they have access too |
| [[ne#yo-ne]] — right? | presents your belief and asks for confirmation | very common in speech; leaves room to be corrected |
| [[n-da#explanatory]] — the thing is | presents the fact as connected to the situation | conversational; explains rather than reports |

-> Contrast: [[ne#agreement]] — where the ね／よ split is drawn in full, from ね's side

### In conversation

- **Short turns are built on it** — いいよ — sure — 違うよ — that's not it — ないよ — there isn't any.
- **だよ is the casual copula form.** Dropping だ altogether — 明日、休みよ — tomorrow's a day off — reads as markedly feminine rather than simply casual.
- **The order with ね is fixed in standard speech.** よね is the combination; the reverse ねよ is not used, which is one reason the pair is learned as a unit.
- **Notices and announcements drop it.** Written information states the fact; addressing the reader with よ belongs to advertising copy, not to signage.
- **ぞ and ぜ occupy the same slot** with a much narrower, markedly masculine range, and neither is a straight substitute.

!!! warning "Pitfall"
    よ says the listener did not have this. Attached to something they plainly did have — repeating what you already told them, or agreeing with what they just said — it says so anyway, and that is what reads as a correction.

    Answering a question catches learners for a different reason. The question has already said the asker does not know, so よ marks the same gap a second time.

    That second marking can be heard as a comment on the asking. 三時からですよ — it starts at three — is fine among friends and risky said upward.

    There is no politer よ to reach for instead. The repair is to drop it: 三時からです answers the question and marks nothing.

!!! warning "English Trap"
    English has no particle for this, so the nearest things to hand are the exclamation mark and a tag like "you know". Neither one is よ.

    An exclamation mark says how loudly or excitedly something is said. よ says who has the information. 電車、来たよ — the train's in — can be delivered perfectly calmly, and "It's here!" can be shouted by somebody who is telling nobody anything.

    The reverse holds too. An English sentence that adds nothing to the hearer — agreeing, repeating, thinking aloud — usually wants no よ at all.
