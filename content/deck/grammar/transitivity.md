Japanese often gives you two different verbs for one event: ドアが開いた — the door opened — describes what happened to the door, and ドアを開けた — someone opened the door — describes what someone did to it.
...
@@ intransitive — what happens by itself

**Form:** `N が V-intrans` · `N は V-intrans` · `N, V-intrans` in conversational ellipsis

- **Intransitive means there is no direct object** — ドアが開いた — the door opened — has the changing door as its subject, while ハルが走った — Haru ran — has an active subject but still no object.
- **A paired intransitive reports the change** — ドアが開いた — the door opened — puts the changing participant on stage without making a causer part of the verb's argument structure; 電気が消えた and 会議が始まった work the same way.
- **The cause can exist, and even be known** — ドアを開けた — someone opened the door — and ドアが開いた — the door opened — can report the same moment. Choosing the intransitive doesn't deny it happened; it just leaves that side undescribed.
- **を after a movement verb does not make it transitive** — 公園を歩く — walk through the park — marks the route, and 家を出る marks the point departed from in the same way. Neither noun is a direct object acted on, and both verbs remain intransitive.
...
```ex
風[かぜ]でドアが開[あ]いた。
en: The door blew open.
lit: the door opened because of the wind
note: The cause sits in the sentence, marked with で, and 開く stays intransitive; the verb never meant "by itself".
bd: 風で|because of the wind / ドアが|the door / 開いた*|opened
...
```dialog
note: A asks about the sound, so B reports the event rather than an opener's action.
A: 何[なん]の音[おと]？ | What's that sound?
B: ドアが開[あ]いた。 | The door opened.
```

```dialog
note: A asks for the causer, so the transitive question is natural; B supplies only the missing person.
A: 誰[だれ]がドア開[あ]けたの？ | Who opened the door?
B: 私[わたし]。 | Me.
```
...
@@ transitive — the action that caused it
...
- **The paired transitive reports the causing action** — 予定を決めた — we set the schedule — and 値段を変えた choose what someone did, rather than only the resulting change.
...
```ex
電気[でんき]を消[け]した？
en: Did you turn the light off?
note: Nobody is named, and the verb still points at a doer; the transitive side of the pair keeps an actor role, and context supplies who.
bd: 電気を|the light / 消した*|turned off
...
|---|---|---|---|
| a door | ドアが開いた — the door opened | ハルがドアを開けた — Haru opened the door | door's change versus Haru's action |
| a meeting | 会議が始まった — the meeting started | ハルカが会議を始めた — Haruka started the meeting | beginning event versus organizer's action |
| a schedule | 予定が決まった — the schedule was set | 予定を決めた — we set the schedule | resulting decision versus deciding action |
...
|---|---|---|
| ～aru ／ ～eru | intransitive ／ transitive | 閉まる／閉める — close · 始まる／始める — start · 決まる／決める — be decided／decide · 変わる／変える — change |
| ～u ／ ～eru | intransitive ／ transitive | 開く／開ける — open · 付く／付ける — be attached／attach · 届く／届ける — arrive／deliver |
...
Some verbs have no lexical partner at all — 走る — run — has no transitive partner meaning "make someone run," and 食べる — eat — has no intransitive partner meaning "become eaten." A missing partner is normal, not a vocabulary gap.
