**Decide what kind of evidence you have, then pick the form for it** — what
your own eyes tell you (appearance), what looks about to happen (imminence),
or what someone else told you (hearsay, a separate pattern below).

-> Contrast: [[sou-da-hearsay]] — the same そうだ spelling, reporting what someone said rather than what the evidence shows

@@ tldr

Read the evidence in front of you and say what it looks like: adjective stem
+ そう for how something seems (美味しそう), verb stem + そう for what's
about to happen (落ちそう). Negatives split by side — なさそう for
appearance, そうにない for events. Hearsay そうだ (plain clause + そうだ)
is a different pattern and not this page.

@@ appearance — how something seems, on sight

**Form:** `A-stem + そう` (美味しい → 美味しそう · 元気 → 元気そう)

- **Visual judgment, still a judgment** — you haven't tasted it yet, which
  is exactly what the form signals.
- **Irregular** — いい → よさそう; ない → なさそう.
- **そう itself carries no register** — 美味しそうです is simply the polite
  version of 美味しそう; politeness comes from だ／です, not from そう.

!!! warning "Pitfall"
    Not for things you've confirmed. Once you've tasted it, it's 美味しい,
    not 美味しそう — keeping そう where you have direct knowledge reads as
    oddly noncommittal.
```dialog
note: what licenses 高そうに見える — B's own eyes are the evidence, before any bill confirms it.
A: この新[あたら]しいレストラン、高[たか]そうに見[み]えるね。 | This new restaurant looks expensive.
B: だよね。でも意外[いがい]と安[やす]いらしいよ。 | Right? But apparently it's surprisingly cheap.
```

@@ imminence — about to happen

**Form:** `V-stem + そう` (落ちる → 落ちそう · 泣く → 泣きそう)

- **On the verge** — the evidence points at an event about to happen.
- **Which reading you get follows from the attachment**: adjective →
  appearance, verb → imminence. Same そう, one machine.
```dialog
note: what licenses 転びそう — B's visible stumble is the evidence, not a report of what happened.
A: あ、危[あぶ]ない、転[ころ]びそう！ | Whoa, careful, you're about to fall!
B: え、大丈夫[だいじょうぶ]、大丈夫[だいじょうぶ]。 | Oh, I'm fine, I'm fine.
```

@@ sou-ni-nai — no sign of it

**Form:** `V-stem + そうにない`

- **The verb-side negative** — 終わりそうにない, no sign it's going to end,
  is stronger than a plain negative guess: there is visible evidence
  against.
- Not the mirror of なさそう, which negates appearance, not likelihood.

@@ sou-ni-naru — came to the brink

**Form:** `V-stem + そうになる`, almost always past: そうになった

- **Came to the brink and didn't** — 転びそうになった, I nearly fell. The
  implicit non-occurrence is why the past tense dominates.

@@ sou-ni-mieru — the explicitly visual impression

**Form:** `A-stem + そうに見える`

- **The impression, explicitly visual** — 高そうに見える, it looks
  expensive. Plain そうだ already means seems, so adding に見える
  foregrounds the at-a-glance nature, often with a faint hedge that looks
  may deceive.

@@ nasasou — the adjective-side negative

**Form:** `A-stem + なさそう` · `N + が + なさそう`

- **The adjective-side negative** — 美味しくなさそう, doesn't look tasty,
  and 問題なさそう, looks like no problem.
- Compare そうにない above for denying that an event looks likely.
