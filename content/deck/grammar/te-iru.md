...
- **With a verb that names a process, ている presents that process as happening right now** — 食べている — is eating — 走っている — is running — puts the listener inside the action while it's still underway.
- **The action has a beginning and an end somewhere off-screen** — ている only claims that the moment described falls somewhere between them.

```ex
今[いま]部屋[へや]を掃除[そうじ]していますから、後[あと]でかけ直[なお]しますね。
en: I'm cleaning my room right now, so I'll call you back later.
note: 掃除しています names what's happening at this exact moment, and that's the whole justification the sentence needs for calling back later.
bd: 今部屋を|right now / 掃除していますから、*|since I'm cleaning / 後でかけ直しますね|I'll call back later
pat: durative V-ている, action in progress
```

@@ resultant-state — the state a change left behind

**Form:** `V-て + いる` (punctual verbs — changes that happen at a moment)

- **With a verb that names an instantaneous change, ている presents the state that change left behind** — 開いている doesn't mean "is opening"; the opening already happened, and ている presents the open state that resulted.
- **死んでいる — is dead, never is dying** — 死ぬ names the moment of death, a change with no duration for anything to be in progress of; ている can only present what came after it.
- **知っている works the same way** — 知る — to come to know — names a punctual change; 知っている is the state of already knowing, not an ongoing act of learning.
- **知っている's own negative isn't 知っていない** — the natural way to say "I don't know" is plain 知らない, not the ている form negated; most other resultant-state verbs negate ている directly and normally instead — 結婚していない — not married — is entirely ordinary.

```eg
punctual verb, resultant reading: 窓[まど]が開[あ]いている | The window is open.
durative verb, progressive reading: 子供[こども]が走[はし]っている | A child is running.
```
...
- **The same ている form covers a repeated pattern, not just one instance of an action or one resulting state** — 毎日走っている — I run every day — reports a habit, not a single run in progress right now.
- **A frequency word usually keeps the habitual reading from collapsing into the progressive one** — 最近よく食べている — I've been eating a lot lately — leans on 最近よく to signal a pattern rather than this exact moment.

```eg
毎日[まいにち]走[はし]っている。 | I run every day.
```

@@ record-status — a standing fact, stated like a record
...
- **ていた places the resulting state, or the ongoing action, at a point in the past rather than now** — 着いたときにはもう電気が消えていた — by the time I arrived, the lights were already off — puts that off state at the moment of arriving, not at the moment of speaking.
- **た alone reports the event; ていた reports the state that event left, viewed from a later point** — 電気が消えた — the light went out — names the event itself; 電気が消えていた — the light was off — describes the state still holding at some reference moment.

```eg
event: 電気[でんき]が消[き]えた。 | The light went out.
...
casual: 電気[でんき]がついてる。 | The light's on.
```

@@ tete — chaining a state into what follows
...
てた is ていた said casually — 開いてた — was open — 走ってた — was running — carrying ていた's past-reference-point reading at conversational register, exactly as てる carries ている's.
...
- **できている presents something as finished and still in that finished state** — 資料ができている — the materials are ready — reports that the preparation already happened and the ready state holds now.
- **It's the resultant-state reading applied to できる itself** — できる names reaching completion; ている presents the completed state as still in effect, the same relationship 開く／開いている has.

```eg
資料[しりょう]がもうできている。 | The materials are already ready.
```
...
Marking an action as finished and irreversible is a separate auxiliary with its own page, not a flavor of ている.

-> Compare: [[te-shimau]] — completion and irreversibility, a different auxiliary entirely

てくれる can attach to ている itself — 待っていてくれる — asking someone to maintain a state, not just perform one action — but the request stack built on it belongs with the rest of that page's benefactive system.

-> Continue with: [[te-ageru-te-kureru-te-morau]] — the request built on a sustained ている state, taught alongside the rest of that page's ladder

### In conversation

Particles fall away as often as the vowel does. ドア、開いてる？ — is the door open? — drops both が and the full ている. 誰か来てるのかな — I wonder if someone's here — stacks the casual contraction with a musing かな.

```dialog
note: A's casual question about a state licenses B's contracted resultative answer.
A: あれ、台所[だいどころ]の電気[でんき]、ついてる？ | Hey, is the kitchen light on?
B: うん、誰[だれ]か来[き]てるのかな。 | Yeah — wonder if someone's here.
```
